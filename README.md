<h1 align="center">🎤 Vocex Run</h1>
<p align="center">
  <img src="https://github.com/user-attachments/assets/355e0248-ea4b-4a5c-ba56-4fb2ba669e6f" width="700"/>
</p>
<p align="center">
  <b>A voice-controlled endless runner where your reflexes — and your voice — decide how far you survive.</b>
</p>
<p align="center">
  <img src="https://img.shields.io/github/stars/ShounakPatra/vocex-run?style=social"/>
  <img src="https://github.com/ShounakPatra/vocex-run/actions/workflows/android.yml/badge.svg"/>
  <img src="https://img.shields.io/badge/version-5.0-blue?style=flat-square"/>
  <img src="https://img.shields.io/badge/platform-Android-green?style=flat-square"/>
  <img src="https://img.shields.io/badge/status-active-success?style=flat-square"/>
</p>

---

## 📥 Download

<p align="center">
  <a href="https://github.com/ShounakPatra/vocex-run/releases/download/v5.0/VocexRun-v5.0.apk">
    <img src="https://img.shields.io/badge/⬇%20DOWNLOAD%20%20VocexRun%20v5.0%20APK-000000?style=for-the-badge&logo=android&logoColor=3DDC84" height="52"/>
  </a>
</p>
<p align="center">
  <img src="https://img.shields.io/badge/Free-No%20Ads-3DDC84?style=flat-square"/>
  <img src="https://img.shields.io/badge/Min%20SDK-Android%208.0%2B-3DDC84?style=flat-square&logo=android&logoColor=white"/>
  <img src="https://img.shields.io/github/downloads/ShounakPatra/vocex-run/total?style=flat-square&label=Total%20Downloads&color=555"/>
</p>
<p align="center">
  <sub>⚠️ Enable <b>"Install unknown apps"</b> on your device before installing.</sub>
</p>

---

## 🎮 Overview

Vocex Run is a real-time voice-controlled endless runner where your microphone replaces traditional controls.

- 🤫 Silence slows movement
- 🗣️ Speaking increases speed
- 💥 Sharp sounds trigger jumps

> There are no buttons. No touch controls. Only your voice.

---

## ⚙️ Core Gameplay

| Voice Input | Action |
|---|---|
| Stay silent | Slow movement |
| Speak normally | Faster movement |
| Sharp sound | Jump |
| High intensity | Maximum speed |

Precision matters more than volume. Timing is everything.

---

## 🧠 Game Modes

| Mode | Best For |
|---|---|
| 🔇 Silent Mode | Quiet environments |
| 🎙️ Normal Mode | Everyday use |
| 😤 Rage Mode | Noisy surroundings |

Each mode adjusts voice thresholds for consistent gameplay across environments.

---

## 🚀 What's New in v5.0

- ✈️ Flying enemies added for late-game difficulty
- 🌤️ Dynamic shadows based on lighting conditions
- ⚡ Near-instant jump responsiveness
- 🔄 Ceiling collision system — no more off-screen glitches
- 🎯 Improved microphone consistency across devices
- ⚙️ Settings menu with sensitivity controls
- 🧩 Performance and gameplay refinements

---

## ✨ Features

- 🎤 Real-time voice-controlled gameplay
- ⚡ Dynamic difficulty scaling
- 🎯 Smooth physics and collision system
- 🔊 Audio-driven feedback
- 🎮 Multiple gameplay modes

---

## 📸 Gameplay Preview

<p align="center">
  <img src="https://github.com/user-attachments/assets/65d2bff2-6f7a-44e1-84e7-12fcd6b6cebb" width="300"/>
</p>

---

## 🛠️ How to Run

```bash
git clone https://github.com/ShounakPatra/vocex-run.git
```

1. Open in **Android Studio**
2. Connect your Android device via USB
3. Grant microphone permission when prompted
4. Hit **Run ▶**

---

## ⚠️ Requirements & Notes

- Android device with a working microphone
- Microphone permission must be granted at runtime
- Performance may vary depending on device microphone sensitivity

---

## 🐛 Issues & Feedback

Found a bug? Got a suggestion?

👉 [Open an issue](https://github.com/ShounakPatra/vocex-run/issues)

---

## 👨‍💻 Author

**Shounak Patra** — [@ShounakPatra](https://github.com/ShounakPatra)

---

## 📄 License

This project is licensed under the [MIT License](LICENSE).

---

<p align="center">
  <i>💀 If you didn't scream while playing… you played it wrong.</i>
</p>
